...
def run_ezr_explainer(data, print_budget=True):
    """Run EZR explainer and return feature importance"""
    #print('-------ezr:-------')
    t1 = time.time()
    stts = []
    count = 0
    ...
    if print_budget: print("ezr Labeling Budget,", rpt*the.Stop)
    print("---------------------")
    ezr_FI = {}
    for f in data.cols.x:
        ezr_FI[f.txt] = np.average([stts[i][f.txt] for i in range(len(stts))])
    w = sum(vf for vf in ezr_FI.values())
    for f in ezr_FI:
        ezr_FI[f] = ezr_FI[f] / w
    ezr_FI["explainer"] = "ezr"
    ezr_FI["run_time"] = (time.time()-t1)
    return ezr_FI, round( count / rpt )
...
def main():
    ...
    for regressor in ["linear", "rf", "svr", "ann", "lgbm"]:
        ezr, shap, rlf, all, anova = [], [], [], [], []
        t1 = time.time()
        for ii in range(repeats):
            random_seed *= 2
            selected_raw_data = Data(csv2(dataset, top_features['ezr'] + targets))
            data, test_data = split_data(selected_raw_data, random_seed)
            b4    = yNums(data.rows, selected_raw_data)
            ezr.append( exp1(selected_raw_data, data, test_data, b4, regressor, 10) )
        ...
        ezr_acc, shap_acc, rlf_acc, all_acc, anova_acc = stats.SOME(txt=f"{regressor},ezr"), stats.SOME(txt=f"{regressor},SHAP"), stats.SOME(txt=f"{regressor},RLF"), stats.SOME(txt=f"{regressor},all"), stats.SOME(txt=f"{regressor},anova")
        ezr_acc.adds(ezr)
        shap_acc.adds(shap)
        rlf_acc.adds(rlf)
        all_acc.adds(all)
        anova_acc.adds(anova)
        records.append(ezr_acc)
