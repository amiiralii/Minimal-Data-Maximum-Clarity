    ...
    with open(file_path, 'r') as f:
        for line in f:
            ...
            if not line or line == '#':
                ...
                continue
            words = line.split(",")
            if "Number of features selected by ezr" in line or "features present in best path" in line:
...
def fs_analyze_results():
    ...
    if not results_dir.exists():
        ...
        return {}
    cols, cols2 = [], []
    for feature_selection in [ "RLF", "SHAP", "ezr", "anova", "all"]:
        for regressor in ["linear", "rf", "svr", "ann", "lgbm", "ezr"]:
...
def opt_process_csv_file(file_path):
    results = []
    chck = False
    with open(file_path, 'r') as f:
        for line in f:
...
def opt_analyze_results():
    ...
    results_df = pd.DataFrame(columns=cols)
    for file_path in results_dir.glob('*.csv'):
        res, features_count, budget = opt_process_csv_file(file_path)
        new_row = {}
        for i in res:
            new_row[i['trt']] = i['mean']
        dataset_name = file_path.name.split('/')[-1][:-4]
...
if __name__ == "__main__":
    fs_desired_order = [ 'RLF_linear', 'SHAP_linear', 'ezr_linear', 'anova_linear', 'all_linear',
                     'RLF_rf', 'SHAP_rf', 'ezr_rf', 'anova_rf', 'all_rf',
                     'RLF_svr', 'SHAP_svr', 'ezr_svr', 'anova_svr', 'all_svr',
                     'RLF_ann', 'SHAP_ann', 'ezr_ann', 'anova_ann', 'all_ann',
                     'RLF_lgbm', 'SHAP_lgbm', 'ezr_lgbm', 'anova_lgbm', 'all_lgbm',
        'data', 'x*', 'x', 'y', 'budget', 'rows']
    
    opt_desired_order = [ 'asIs', 'linear', 'rf', 'svr', 'ann', 'lgbm', 'ezr',
        'data', 'x*', 'x', 'y', 'budget', 'rows']
    
    results, colors, all = fs_analyze_results()
    results = results.sort_values(by=['x','data'])
    results[fs_desired_order].to_csv('../results/fs_results.csv', index=False)

    results = opt_analyze_results()
    results = results.sort_values(by=['x','data'])
    results[opt_desired_order].to_csv('../results/optimization_results.csv', index=False)
